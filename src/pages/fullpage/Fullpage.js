...
const Fullpage = () => {
  ...
  useEffect(() => {
    const options = {
      ...
      rootMargin: '10px', //root가 가질 margin값으로 root의 측면마다 경계의 크기를 조절할 수 있다. 기본 값은 0
      threshold: 0.5,
      // 관찰자가 root와 target간의 교차가 이루어졌다고 판단하는 기준이다. 0.5는
    ...
    const observers = sectionRef.current.map((ref, i) => {
      ...
      return new IntersectionObserver(([ref]) => {
